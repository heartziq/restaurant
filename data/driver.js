 ...
 */
 ...
 function filterOpenNow(db) {
   ...
   const now = moment({hour: timeNow, minute: minuteNow })
                .format();
      ...
      for (let each of e.open) {
        if (each.day.includes(moment().day())) {
          const { hour: hourStart,
                  minute: minuteStart } = convertTo24(each.time.openAt);

          const { hour: hourEnd,
                  minute: minuteEnd } = convertTo24(each.time.closeAt);
                  
          if (now >= moment({ hour: hourStart, minute: minuteStart}).format() && now < moment({ hour: hourEnd, minute: minuteEnd}).format())
            return true
        }
      }
    })
...
function opensAfter(db, time) {
  ...
  const openThisDay = db.filter(e => {
    for (let each of e.open) {
      const { hour: hourEnd, minute: minuteEnd } = convertTo24(each.time.closeAt);
...
function openAtDay(db, day) {
  ...
  const dayInAbbreviation = day[0].toUpperCase().concat(day.slice(1, 3).toLowerCase());
...
function getNow() {
  return moment({ day: moment().day(), hour: moment().hour(), minute: moment().minute() }).format();
}

function isOpenNow() {
  const now = getNow();
  const target = moment({day: daysOfWeek['Mon'], hour: 22, minute: 30}).format();
...
function getComplete(dayRange) {
  if (dayRange.includes(',')) {
    return dayRange.replace(/\s+/, '').split(',').reduce((acc, ele) => acc.concat(getDayRange(ele)), []);
  }
